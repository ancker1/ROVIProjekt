...
void SamplePlugin::initialize() {
	...
	// Auto load workcell
    WorkCell::Ptr wc = WorkCellLoader::Factory::load("../../Project_WorkCell/Scene.wc.xml");
...
void SamplePlugin::getImageNamed(std::string name) {
    if (_framegrabber != NULL) {
        for(unsigned int i = 0; i < _cameras.size(); i ++)
        {
            // Get the image as a RW image
            ...
            cv::imwrite("/home/mikkel/Desktop/Project_WorkCell_Cam/performance_pic/" + _cameras[i] + name + ".png", imflip_mat );
...
void SamplePlugin::generateTestImagesBall() {
    std::ifstream myFile;
    myFile.open("/home/mikkel/Desktop/Project_WorkCell_Cam/performEval_ball_pos.txt");

    rw::math::Vector3D<> temp_pos;
    std::vector<rw::math::Vector3D<>> ball_test_positions;

    std::string line;
    std::string temp;
    unsigned int coordinate = 0;
    // Reading test positions for ball
    while(myFile >> line)// read lines
    {
        temp = "";
        coordinate = 0;
        ...
        ball_test_positions.push_back(temp_pos);
    }
    // Moving the ball to test position and take pictures
    for(unsigned int i = 0; i < ball_test_positions.size(); i++)
    {
        rw::kinematics::MovableFrame *ball = _wc->findFrame<rw::kinematics::MovableFrame>("Ball");
        rw::math::Vector3D<> posBall;
