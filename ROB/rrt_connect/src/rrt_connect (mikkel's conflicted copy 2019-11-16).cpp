...
{
    // Running 6 threads when writing to file
...
//    myfile.close();
    ...
    /****************************************************************************************
    **                 Calculating data for optimal parameter stepsize                     **
    ****************************************************************************************/
    /*
    ...
    ****************************************************************************************/
    ...
    for (unsigned int i = 0; i < number_of_data; i++)
    {
        ...
        if (i == 0) // Do not append at the start to file
            calc_and_print_path_treaded(false, stepsizes, wc, device, tool_frame, cylinder_frame, from, to); // Running 6 threads when generating data
        else
            calc_and_print_path_treaded(true, stepsizes, wc, device, tool_frame, cylinder_frame, from, to); // Running 6 threads when generating data
    }
	return 0;
}
